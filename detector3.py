...
def isDuplicateId(id, activity,actYear):
    Log = db["Log"]
    print(id,activity)
    x = Log.find_one({"id":id ,"activity":activity,"actYear":actYear})
    print ("x",x)
    if  x==None:
        print("Not duplicate")
        return False
...
def send(id):
    ...
    count_data = getCurrentCountByActivity(activity[0],actYear)
    
    # print(activity)
    date = datetime.now()
    if(activity[2] == 1):
        post = {"name": name, "id": id, "date": date,"activity":activity[0],'actYear':actYear,'weing':weing}        
        urlform ="http://localhost:3000/assessmentform/{}/{}/{}".format(activity[0],id,actYear);
...
def getCurrentCountByActivity(activity,actYear):
    currentCountData =0
    data = db["data"]
    # filters = {'ActivityName':activity
    # }
    x = list(data.aggregate([
        { 
          '$match': {
            '$expr': 	{  
          '$and':[
                {'$eq': [{ '$year': '$startTime' },  actYear ]},
                {'$eq': ['$ActivityName',  activity ]},
               ]
              }
